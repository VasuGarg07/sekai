...
const NavMenu = ({ className = '' }) => {
    ...
    return (
        <div className={className} ref={menuRef}>
            ...
            {open && (
                <div className="absolute mt-2 min-w-48 bg-zinc-800 rounded-lg shadow-lg border border-zinc-700 py-2 z-50
                    ...
                    {navigationLinks.map((link) => (
                        ...
                        >
                            <link.icon size={16} className="text-accent-500 transition-colors duration-200" />
                            <span>{link.name}</span>
                        </Link>
                    ))}
                </div>
