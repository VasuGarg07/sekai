    ...
    return (
        <div className={`relative max-w-xl ${className}`}>
            ...
            {query.length > 0 && query.length < 3 && (
                <div className="absolute left-0 right-0 mt-1 p-4 text-center text-zinc-400 bg-zinc-800 rounded-lg border border-zinc-700 z-50">
                    Type at least 3 characters to search
                </div>
            )}

            {shouldFetch && (
                <div className="absolute left-0 right-0 mt-1 bg-zinc-800 rounded-lg shadow-xl border border-zinc-700 max-h-96 overflow-y-auto z-50">
                    {isLoading ? (
